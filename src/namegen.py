    ...
    def generate_once(self, target_len, max_len, stop_prob, rng):
        ...
        if m.order == 1:
            if not m.root.next_counts:
                return ""
            first = sample_weighted(m.root.next_counts, rng)
            if first is None:
                return ""
            name_chars = [first]
        else:
            if not m.start_counts:
                return ""
            start_ctx = sample_weighted(m.start_counts, rng)
        ...
        while len(name_chars) < max_len:
            if target_len is not None and len(name_chars) >= target_len:
                break
            if target_len is None and rng.random() < stop_prob and "".join(name_chars) in m.names:
                break
